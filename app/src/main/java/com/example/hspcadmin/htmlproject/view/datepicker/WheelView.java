...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xF0FFFFFF; //选中字体颜色白版

	private static final int VALUR_TEXT_WHITE_COLOR=0xF0333333;//选中字体颜色黑版
	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0xF0474959; //未选中的字体颜色黑版
	private static final int ITEMS_TEXT_COLOR_WHITE = 0xF09A9A9A; //未选中的字体颜色白版
	/** Top and bottom shadows colors */
	private static final int[] SHADOWS_COLORS = new int[] { 0xFF212233, 0x00212233, 0x00212233 };
	private static final int[] SHADOWS_COLORS_WHITE = new int[] { 0xFFFFFFF, 0xFFFFFFF, 0xFFFFFFF };
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			itemsPaint.setTextSize(getContext().getResources().getDimension(R.dimen.text_size_15));
		}

		if (valuePaint == null) {
			valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.FAKE_BOLD_TEXT_FLAG | Paint.DITHER_FLAG);
			valuePaint.setTextSize(getContext().getResources().getDimension(R.dimen.text_size_15));
			valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
		}

		if (centerDrawable == null) {
			//中间选中条目背景色
			if (AppException.getInstance()
					.getSharedPreferencesValue(AppException.SP_THEME).equals("1")) {
				//白天模式
				centerDrawable = getContext().getResources().getDrawable(R.drawable.wheel_val_white);
		...
		if (topShadow == null) {
			if (AppException.getInstance()
					.getSharedPreferencesValue(AppException.SP_THEME).equals("1")) {
				//白天模式
				topShadow = new GradientDrawable(Orientation.TOP_BOTTOM, SHADOWS_COLORS_WHITE);
		...
		if (bottomShadow == null) {
			if (AppException.getInstance()
					.getSharedPreferencesValue(AppException.SP_THEME).equals("1")) {
				//白天模式
				bottomShadow = new GradientDrawable(Orientation.BOTTOM_TOP, SHADOWS_COLORS_WHITE);
...
//		setBackgroundResource(R.drawable.wheel_bg);
	...
	private void drawValue(Canvas canvas) {
		if (AppException.getInstance()
				.getSharedPreferencesValue(AppException.SP_THEME).equals("1")) {
			//白天模式
			valuePaint.setColor(VALUR_TEXT_WHITE_COLOR);
		} else {
			//夜间模式
		...
		if (valueLayout != null) {
			canvas.save();
			if(WHEE_Y == 0){
				WHEE_Y = bounds.top + scrollingOffset;
			}
	...
	private void drawItems(Canvas canvas) {
		...
		if (AppException.getInstance()
				.getSharedPreferencesValue(AppException.SP_THEME).equals("1")) {
			//白天模式
			itemsPaint.setColor(ITEMS_TEXT_COLOR_WHITE);
		} else {
			//夜间模式
	...
	private void drawCenterRect(Canvas canvas) {
		...
		if(WHEE_Y!=0){
			centerDrawable.setBounds(0, WHEE_Y - ToolUtils.dpToPx(2), getWidth(), WHEE_Y + ToolUtils.dpToPx(15)+ ToolUtils.dpToPx(4));
		}else {
			centerDrawable.setBounds(0, center - offset - ToolUtils.dpToPx(5), getWidth(), center - ToolUtils.dpToPx(5));
		}
